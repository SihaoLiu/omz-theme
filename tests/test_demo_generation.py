...
class DemoGenerationTest(unittest.TestCase):
    ...
    def test_termination_removes_the_named_docker_renderer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp) / "project"
            fake_bin = project / "bin"
            scripts_dir = copy_demo_sources(project)
            ...
            try:
                ...
                while not ready_file.exists() and generated.poll() is None:
                    self.assertLess(time.monotonic(), deadline)
                    time.sleep(0.01)
                self.assertTrue(ready_file.exists(), "docker renderer did not start")
                renderer_pid = int(renderer_pid_file.read_text(encoding="ascii"))
                child_pid = int(child_pid_file.read_text(encoding="ascii"))
    ...
    def test_renderer_does_not_receive_preexisting_scratch_contents(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(
                any(argument.startswith(f"{stale_dir}:") for argument in arguments)
            )
            self.assertEqual(
    ...
    def test_fixture_preparation_rejects_existing_symbolic_link_nodes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            victim = root / "victim"
            victim.write_text("keep\n", encoding="ascii")
            (fixture_dir / "session.sh").symlink_to(victim)
    ...
    def test_local_renderer_uses_only_the_synthetic_environment(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp) / "project"
            fake_bin = project / "bin"
            scripts_dir = copy_demo_sources(project)
...
""",
            ...
            fixture = Path(values["HOME"]).parent
    ...
    def test_container_renderer_disables_networking(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp) / "project"
            fake_bin = project / "bin"
            scripts_dir = copy_demo_sources(project)
...
""",
            ...
            self.assertIn("HOME=/work/home", arguments)
            self.assertIn("XDG_CACHE_HOME=/work/cache", arguments)
            self.assertIn("XDG_CONFIG_HOME=/work/config", arguments)
            self.assertIn("USER=demo", arguments)
            self.assertNotIn(f"{project}:/vhs:ro", arguments)
            self.assertIn(f"{scripts_dir / TAPE.name}:/tape/demo.tape:ro", arguments)
            render_mount = next(
                argument for argument in arguments if argument.endswith(":/work:rw")
            )
            render_source = Path(render_mount.removesuffix(":/work:rw"))
            self.assertEqual(project / "temp", render_source.parent)
            self.assertTrue(render_source.name.startswith("demo-render."))
    ...
    def test_fixture_is_synthetic_network_free_and_interactive(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            session = fixture_dir / "session.sh"
            self.assertTrue(session.is_file())
            subprocess.run(["sh", "-n", str(session)], check=True)
    ...
    def test_committed_demo_assets_have_expected_formats(self) -> None:
        ...
        self.assertEqual((1600, 360), struct.unpack("<HH", gif_data[6:10]))
        self.assertGreater(gif_data.count(b"\x21\xf9\x04"), 1)

        self.assertEqual(b"\x89PNG\r\n\x1a\n", png_data[:8])
        self.assertEqual(b"IHDR", png_data[12:16])
        self.assertEqual((1600, 360), struct.unpack(">II", png_data[16:24]))
